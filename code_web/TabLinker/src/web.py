...
def trust():
    ...
    return template('trust-service', state='start', inFiles=inFiles, outFiles=outFiles)
    #return inPath
    #return "Hello World!"
...
def do_upload():
	...
	test_data.save(save_path, overwrite = True)
        ...
        if train_data is not None and train_name is not None and test_data is not None and test_name is not None:
            ...
            return template('trust-service', state='uploaded')
...
def tablinker():
    ...
    for filename in files :
        ...
        logging.info("Done")
    #return "Converted into rdf"
    return template('trust-service', state='converted', numtriples=str(len(tLinker.graph)))



@route('/trust/download')
def download():
...
def output():
    #fileDownload = fileList[-1].split('.')[0] + '.ttl'
    #return static_file(fileDownload, root = '../output/', download = fileDownload)
    os.system("trust.py") # TODO: 
    #return static_file('output.txt',root = '../output')
    return static_file('output.txt',root = '../output')
